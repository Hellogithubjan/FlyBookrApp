...
if(isset($_POST['flight_but']) and isset($_SESSION['adminId'])) {
    ...
    if($flag == false) {
      ...
      if($row = mysqli_fetch_assoc($result)) {
        ...
        $sql = "INSERT INTO Flight (admin_id, arrival, departure, Destination, source, airline, Seats, duration, Price, status, issue, flight_img) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, '', '', ?)";
        ...
        if(!mysqli_stmt_prepare($stmt,$sql)) {
          ...
          mysqli_stmt_bind_param($stmt, 'isssssisis', $admin_id, $arrival, $departure, $arr_city, $dep_city, $airline_name, $seats, $dura, $price, $img);
          mysqli_stmt_execute($stmt);
        }
        mysqli_stmt_close($stmt);
        mysqli_close($conn);
