...
if(isset($_POST['pass_but']) && isset($_SESSION['userId'])) {
    ...
    for($i=0;$i<$date_len;$i++) {
        $sql = 'INSERT INTO Passenger_profile (user_id,mobile,dob,f_name,l_name,flight_id) VALUES (?,?,?,?,?,?)';            
        if(!mysqli_stmt_prepare($stmt,$sql)) {
            header('Location: ../pass_form.php?error=sqlerror');
            exit();            
        } else {
            mysqli_stmt_bind_param($stmt, 'sissss', $_SESSION['userId'], $_POST['mobile'][$i], $_POST['date'][$i], $_POST['firstname'][$i], $_POST['lastname'][$i], $flight_id);                          
            mysqli_stmt_execute($stmt);  
            $flag = true;        
        }
    }   
